...
function PlatformButtons({ messageKey, large }: { messageKey: EnrollMessageKey; large?: boolean }) {
  ...
  const btn = large ? 'py-3.5 text-sm' : 'py-2.5 text-xs sm:text-sm';
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-2.5">
      <a
        ...
        className={`group flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#25D366] px-4 font-bold text-white shadow-lg shadow-[#25D366]/20 transition-all duration-300 hover:brightness-110 hover:scale-[1.02] hover:shadow-[#25D366]/35 ${btn}`}
      >
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className={`group flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#0088cc] px-4 font-bold text-white shadow-lg shadow-[#0088cc]/20 transition-all duration-300 hover:brightness-110 hover:scale-[1.02] hover:shadow-[#0088cc]/35 ${btn}`}
      >
        <svg className="h-4 w-4 shrink-0 transition-transform group-hover:scale-110" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
...
export function EnrollmentSection() {
  ...
  return (
    <section
      id="enroll"
      className="relative py-28 px-6 overflow-hidden scroll-mt-20 bg-gray-900/50 border-y border-gray-800/60"
    >
      {/* Ambient — matches hero */}
      <div className="absolute top-1/4 -left-32 w-[420px] h-[420px] rounded-full bg-emerald-500/10 blur-[110px] animate-float-a pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-[380px] h-[380px] rounded-full bg-teal-500/8 blur-[100px] animate-float-b pointer-events-none" />
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <EnrollReveal className="text-center mb-12">
          <div className="inline-flex items-center gap-2.5 mb-5 rounded-full border border-emerald-500/30 bg-emerald-500/8 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.22em] text-emerald-400 animate-shimmer">
        ...
        {/* Stat pills */}
        <EnrollReveal delay={80} className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-12">
          {[
            { label: 'Starts', value: '8 June 2026', accent: 'text-teal-300 border-teal-500/25 bg-teal-500/10' },
            { label: 'Seats left', value: 'Only 10', accent: 'text-emerald-300 border-emerald-500/30 bg-emerald-500/10 enroll-seat-pulse' },
            { label: 'Reserve with', value: '₹2,000', accent: 'text-amber-300 border-amber-500/25 bg-amber-500/10' },
          ].map((pill) => (
            <div
              key={pill.label}
              className={`rounded-2xl border px-5 py-3 text-center min-w-[120px] backdrop-blur-sm transition-transform hover:-translate-y-0.5 ${pill.accent}`}
            >
              <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">{pill.label}</p>
              <p className="text-lg font-extrabold text-white mt-0.5">{pill.value}</p>
            </div>
          ))}
        </EnrollReveal>
        ...
        <EnrollReveal delay={120}>
          <div className="relative rounded-3xl bg-gray-950/80 border border-gray-800/80 overflow-hidden card-gradient-border shadow-2xl shadow-black/40">
            ...
            <div className="relative grid lg:grid-cols-5 gap-0">
              ...
              <div className="lg:col-span-3 p-6 sm:p-8 space-y-6 bg-gray-900/30">
                <div className="rounded-2xl border border-emerald-500/25 bg-gradient-to-br from-emerald-950/50 to-gray-900/80 p-5 sm:p-6">
                  ...
                  <div className="flex items-center gap-3 rounded-xl bg-black/50 border border-gray-700/80 px-4 py-3.5 group hover:border-emerald-500/40 transition-colors">
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-0.5">UPI ID</p>
                      <p className="font-mono text-base sm:text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-300 to-teal-300 truncate">
                        {UPI_ID}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={copyUpi}
                      className={`shrink-0 rounded-lg px-4 py-2.5 text-xs font-bold transition-all duration-300 ${
                        copied
            ...
            {/* CTA strip */}
            <div className="relative border-t border-gray-800/80 bg-gray-950/60 p-6 sm:p-8 grid sm:grid-cols-2 gap-5">
              {CTAS.map((cta, i) => (
                <div
                  key={cta.key}
